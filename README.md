...
- 📊 **监控池管理** — 自由添加/删除申万行业指数与国际指数标的；行业指数支持四种策略分类与成分股查看，国际指数采用趋势跟踪策略
...
| **国际指数** | 价格 + MA60/MA120 趋势 | 站上 MA60 且 MA60>MA120（多头排列）→ 买入 | 跌破 MA60 或 MA60 下穿 MA120 → 减仓 |
...
> **提示：** 系统实际使用的 Tushare 接口及权限要求：`sw_daily`（申万行业指数行情）、`index_global`（国际指数行情，需 **6000 积分**）、`index_classify`（行业分类）、`trade_cal`（交易日历）、`idx_factor_pro`（技术因子，需 **5000 积分**）、`index_member_all`（成分股，需 **2000 积分**）。因子/成分股接口无权限时对应功能降级，不影响核心巡检。
...
7. **消息推送（可选）** — 在侧边栏「消息推送设置」中添加你的 Webhook 地址并开启开关；默认仅推送 AI 建议为「买入/卖出」的强信号（可在设置中关闭过滤恢复全量推送）；定时巡检仅推送你监控的指数，且推送内容为你的提示词配置对应的专属分析结果
